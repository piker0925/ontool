...
| 🟪 | **`/fun` 재미·게임** | 로또 · 랜덤 도구 · 워드클라우드 · 룰렛 · 밸런스게임 · 미니게임 16종 | 29 |
...
- **게임** (16) — 반응속도 · 2048 · 지뢰찾기 · 카드 짝맞추기 · 스네이크 · 사이먼 · 숫자야구 · 틱택토 · 워터소트 퍼즐 · 타워 쌓기 · 블록 블라스트 · 매치3 퍼즐 · 벽돌깨기 · 두더지잡기 · 장애물 피하기 · 슬라이딩 퍼즐. 게임 자체는 프론트 로컬이지만, 로그인 사용자의 점수는 전부 백엔드 `game` 모듈(세션 토큰·레이트리밋·물리적 상한 검증)로 제출돼 리더보드에 반영된다.
