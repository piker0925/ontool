...
| 🟧 | **`/life` 생활 도구** | 급여 · 금융 · 건강 · 날짜 · 단위 계산기 · 텍스트/조회 | 36 |
| 🟪 | **`/fun` 재미·게임** | 로또 · 랜덤 도구 · 워드클라우드 · 룰렛 · 밸런스게임 · 미니게임 18종 | 31 |
...
1. **외부 인프라 없이 MySQL만으로 만든 비동기 작업 큐** — Redis·RabbitMQ 없이 `Job` 테이블 + `SELECT ... FOR UPDATE SKIP LOCKED`로 다중 워커가 동시에 꺼내가도 중복 처리 없는 큐를 구현하고, **레인(Lane) 기반 동시성 제어**(영상 인코딩=동시 1, 짧은 작업=동시 2)와 소유자별 공정 스케줄링·용량 인지 admission으로 2코어 무료 VM에서도 무거운 작업이 큐를 막지 않게 했다. ([설계 →](docs/adr/0019-job-scheduler-resource-model.md)) — **실측**: 소유자 2명까지는 공정 스케줄링이 정확히 작동(뒤에 온 소유자가 앞선 40건 백로그와 무관하게 2번째 틱에 서비스됨)하고, 과부하 시 503/507로 즉시·깔끔하게 거절됨을 로컬 벤치마크로 확인했다 — permit 수보다 소유자가 많을 때 뒤쪽 소유자가 굶는 경계도 같은 실험에서 발견했고(→ [트러블슈팅](#트러블슈팅)), 레인별 회전(rotation) 방식으로 고쳐 재측정까지 마쳤다(첫 서비스 56.6초→4.0초). ([방법론·원본 데이터 →](docs/benchmarks/103-heavy-queue-bench/README.md))
...
3. **공통 카탈로그로 백엔드 37개·프론트 96개, 133개 도구를 통합한 확장 구조** — 백엔드 도구는 `ToolModule`을 구현하는 `@Component` 클래스 하나만 추가하면 자동 등록되고, 순수 계산으로 끝나는 나머지 96개는 백엔드를 거치지 않고 프론트에서 바로 처리된다. 성격이 다른 파일 처리·즉석 계산·게임·문서 뷰어가 같은 카탈로그·검색·통계 위에 공존하되, 행위 공유는 일부러 하지 않아(과추상화 방지) 데이터 계층만 통합했다. ([설계 →](docs/adr/0026-page-module-generalization.md))
...
> **테스트**: 백엔드 710+ (JUnit 5 · Testcontainers MySQL · Awaitility) · 프론트엔드 1,690+ (vitest). 주요 결정은 [ADR 34건](docs/adr/)으로 기록.
...
- **단위·변환** (4) — 단위 변환 · 택배 부피무게 · 타임존 변환 · 유류비
- **날짜·나이** (6) — 반려동물 나이 · D-Day/날짜 차이 · 만 나이 · 육아 개월수 · 출산예정일 · 전역일
- **텍스트** (5) — Diff 비교 · 한글 이름 로마자 변환기 · 특수문자·이모지 모음 · 한영타 변환기 · 글자수 세기
- **생산성** (1) — 뽀모도로 타이머
...
### `/fun` 재미·게임
- **재미** (13, 전부 프론트 로컬) — 로또 번호 생성 · 로또 시뮬레이터 · 랜덤 팀/사다리타기 · 랜덤 닉네임 · 색상 팔레트 생성기 · 워드클라우드 · 바이오리듬 · 혈액형·이름 궁합 · 룰렛 돌림판 · 순서 정하기 · 핀볼 추첨기 · 밸런스 게임 · 아이스브레이킹 질문
- **게임** (18) — 반응속도 · 2048 · 지뢰찾기 · 카드 짝맞추기 · 스네이크 · 사이먼 · 숫자야구 · 틱택토 · 워터소트 퍼즐 · 방치형 클리커 · 단어 맞추기 · 타워 쌓기 · 블록 블라스트 · 매치3 퍼즐 · 벽돌깨기 · 두더지잡기 · 장애물 피하기 · 슬라이딩 퍼즐. 게임 자체는 프론트 로컬이지만, 앞의 8종은 로그인 사용자의 점수가 백엔드 `game` 모듈(세션 토큰·레이트리밋·물리적 상한 검증)로 제출돼 리더보드에 반영된다 — 나머지 10종(121)은 아직 리더보드 미연동.
...
- **공정 스케줄링 + 쿼터**: 한 사용자가 대량으로 밀어넣어도 다른 사용자가 굶지 않도록 소유자별 라운드로빈으로 꺼내고, 전역 용량(디스크 예산·큐 깊이)을 문 앞에서 검사해 초과 시 거절한다(ADR-0019, ADR-0021). **실측(로컬)**: HEAVY 레인(permit=2)에서 소유자 2명은 공정성이 정확히 성립(뒤에 온 소유자가 40건 백로그와 무관하게 2번째 틱에 서비스)한다. 소유자 3명부터는(permit 수보다 owner 수가 많음) 127 수정 전에는 뒤쪽 owner가 앞선 두 owner의 백로그(약 60초 동안 40건)가 전부 빌 때까지 단 한 틱도 선택되지 못했으나, 레인별 "마지막으로 서비스한 소유자"를 기억해 다음 틱은 그 다음 소유자부터 순회하도록 회전시키는 수정 후에는 두 번째 틱(약 4초) 안에 서비스된다 — 자세한 경위·before/after 수치는 [트러블슈팅](#트러블슈팅) 참고. 과부하 시에는 큐 깊이 상한을 넘는 요청이 정확히 12번째에서 503으로, 디스크 예산 초과는 507로 즉시·깔끔하게 거절됨을 확인했다(시스템이 느려지거나 큐가 무한정 쌓이지 않음). ([방법론·원본 데이터 →](docs/benchmarks/103-heavy-queue-bench/README.md))
...
### 라운드로빈 공정성의 미문서화 경계 (owner 수 > permit 수)

큐·레인 동시성을 README에 "실측"으로 뒷받침하려고 로컬 부하 벤치마크(이슈 103)를 설계하던 중,
"HEAVY 레인을 소유자 2명으로만 테스트하면 항상 통과해버려서 공정성이 항상 성립한다고 잘못 결론
내릴 위험"이 그릴링 과정에서 지적됐다. 실제로 `JobWorker.selectFair`를 다시 읽어보니:
...
owner를 순회하다 `chosen.size() >= limit`가 되는 순간 그 라운드의 순회를 즉시 멈춘다. permit(=limit)이
owner 수보다 적으면, 매 틱 **맵에 먼저 등록된 owner들만** 한 건씩 골라지고 뒤쪽 owner는 앞선 owner들의
백로그가 완전히 빌 때까지 단 한 번도 선택되지 못한다 — "라운드로빈이니 굶지 않는다"는 문서화된 주장은
**owner 수 ≤ permit 수**일 때만 성립하는 것이었다.

로컬 벤치마크로 재현한 결과([시나리오 2](docs/benchmarks/103-heavy-queue-bench/README.md#2-heavy-레인permit2-소유자-3명--공정성-경계-밖-케이스-127-이전-baseline)):
소유자 A·B가 각 20건씩 투입 후 C가 3건 늦게 투입하면, C는 A+B의 백로그가 거의 다 빌 때까지(약 57초 동안)
단 한 건도 PENDING을 벗어나지 못했다. HEAVY 레인은 permit=2라 실사용(동시 접속자 3명 이상)에서 충분히
발생할 수 있는 경계다.

**해결(이슈 127)**: `JobWorker`에 레인별 `lastServedOwner`(마지막으로 서비스한 소유자)를 기억하는
인메모리 상태를 추가하고, `selectFair`가 owner 순회를 매번 처음부터 시작하는 대신 그 다음 소유자부터
시작하도록(회전) 고쳤다. 단일 워커 인스턴스에서 `poll()`이 `@Scheduled(fixedDelay)`로 직렬 실행되므로
새 동시성 보호 장치 없이 plain `Map`으로 충분하다(ADR-0019가 이미 확정한 "단일 워커라 in-memory 선택으로
충분" 전제를 그대로 따름). 같은 하네스로 재측정한 결과, C의 첫 작업이 PENDING을 벗어나는 시각이
**56.6초 → 4.0초**로, A·B가 여전히 40건 중 34건 이상 남아있는 상태에서도 서비스됨을 확인했다
([시나리오 2-후속](docs/benchmarks/103-heavy-queue-bench/README.md#2-후속-heavy-레인permit2-소유자-3명--127-수정-후-재측정-beforeafter)).
자세한 동시성·정합성 분석은 [ADR-0019 "공정성 보장의 경계"](docs/adr/0019-job-scheduler-resource-model.md#공정성-보장의-경계) 참고.

방법론·원본 로그 전체는 [`docs/benchmarks/103-heavy-queue-bench/`](docs/benchmarks/103-heavy-queue-bench/)에 보존했다.
