...
C를 끼워 넣었다. A+B의 총 소요 시간(59.1초→70.1초)의 방향(약간 늘어남)은 이론상 설명이 된다 —
매 틱 permit 2개 중 하나를 C에게 양보하는 틱이 있으면 그만큼 A·B 몫 처리가 뒤로 밀린다(총 작업
43건을 permit 2개로 처리하는 총량 자체는 거의 동일, 순서가 owner 단위 FIFO에서 진짜 라운드로빈으로
바뀐 것뿐이다). 다만 **C가 옮긴 건 3건뿐이라 이론상 밀림은 수 초 수준이어야 하는데 실측 차이(11초)는
그보다 크다** — 이 측정을 다른 워크트리의 병렬 테스트·Testcontainers들과 동시에 로컬 CPU를 공유한
상태로 돌렸으므로(§환경), 차이의 상당 부분은 회전 로직 자체가 아니라 **측정 시점의 로컬 부하 변동**일
가능성이 높다. 절대 처리량이 스코프 밖이라는 이 문서의 원칙과 같은 이유로, 이 11초 차이를 회전
로직의 비용으로 단정하지 않는다 — 격리된 환경에서 재측정하면 좁혀질 수 있는 수치다. 원본 데이터:
