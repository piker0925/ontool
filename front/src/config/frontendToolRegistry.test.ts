...
describe('FRONTEND_TOOL_COMPONENTS — 등록 일관성', () => {
    // FULL_SHELL_COMPONENTS가 이미 담당하는 id(게임뿐 아니라 뽀모도로처럼 GamePage 셸만 재사용하는
    // 생활 도구도 포함 — kind로 구분하지 않는다, ADR-0026: kind는 렌더 결정권이 없다)는 대상에서 제외.
    const frontendOnlyIds = MOCK_MODULES.filter(m => m.isFrontendOnly && !(m.id in FULL_SHELL_COMPONENTS)).map(m => m.id)
