...
## 알려진 한계 — 인스턴스를 늘리면 제한이 약해짐

`RateLimiter`는 IP별 카운터를 Guava `Cache`(프로세스 인메모리)로 들고 있다. 단일 인스턴스에서는 문제없지만, 백엔드를 여러 인스턴스로 스케일아웃하면 로드밸런서가 같은 IP의 요청을 인스턴스마다 나눠 보내고 각 인스턴스가 카운트를 독립적으로 세기 때문에, 실질 한도가 인스턴스 수만큼(N배) 느슨해진다 — 예를 들어 인스턴스 3대라면 IP당 60초/200회 한도가 사실상 60초/600회에 가까워질 수 있다.

이 프로젝트의 다른 상태 있는 컴포넌트들(`AdmissionControl`은 매 요청마다 DB·파일시스템을 직접 조회해 정적으로 판정, 만료 정리 스케줄러는 전부 멱등)과 달리 `RateLimiter`만 유일하게 인스턴스별로 갈라지는 상태를 갖는다. 근본 해결책은 Redis 같은 공유 저장소 기반 리미터로 교체하는 것이지만, 지금은 단일 인스턴스로만 운영하고(스케일아웃 계획 없음) 이 프로젝트 전체가 분산 락·공유 상태 조율 장치를 의도적으로 배제하고 있어(Job 큐는 SKIP LOCKED로 DB가 유일한 조율점) 새 인프라를 들이지 않는다. 인스턴스를 늘릴 계획이 생기면 그때 이 한계가 재검토 대상이다.
