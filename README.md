...
| | 구역 | 대표 도구 | 도구 수 |
|:--:|---|---|:--:|
| 🟦 | **`/dev` 개발자 도구** | 포맷터 · 인코더 · 데이터 변환 · 코드 생성 · 보안 · 네트워크 · 텍스트 | 37 |
| 🟩 | **`/files` 파일·문서** | PDF · 이미지 · 영상 · 오디오 · 오피스 문서 변환 | 33 |
| 🟧 | **`/life` 생활 도구** | 급여 · 금융 · 건강 · 날짜 · 단위 계산기 · 재미 · 로또 · 랜덤 도구 | 42 |
| 🟪 | **`/fun` 게임** | 미니게임 15종 | 15 |
...
3. **공통 카탈로그로 백엔드 37개·프론트 90개, 127개 도구를 통합한 확장 구조** — 백엔드 도구는 `ToolModule`을 구현하는 `@Component` 클래스 하나만 추가하면 자동 등록되고, 순수 계산으로 끝나는 나머지 96개는 백엔드를 거치지 않고 프론트에서 바로 처리된다. 성격이 다른 파일 처리·즉석 계산·게임·문서 뷰어가 같은 카탈로그·검색·통계 위에 공존하되, 행위 공유는 일부러 하지 않아(과추상화 방지) 데이터 계층만 통합했다. ([설계 →](docs/adr/0026-page-module-generalization.md))
...
- **텍스트** (7) — Regex 테스터 · 마크다운 도구(TOC·표) · 텍스트 유틸(케이스·글자수·라인) · Diff 비교 · 한글 이름 로마자 변환기 · 한영타 변환기 · 글자수 세기
...
- **텍스트** (1) — 특수문자·이모지 모음 (텍스트 카테고리 중 재미 성격이 강해 예외적으로 잔류, 나머지는 `/dev`로 이동)
- **재미** (12) — 로또 번호 생성 · 로또 시뮬레이터 · 랜덤 팀/사다리타기 · 랜덤 닉네임 · 색상 팔레트 생성기 · 워드클라우드 · 룰렛 돌림판 · 순서 정하기 · 핀볼 추첨기 · 밸런스 게임 · MBTI 궁합 계산기 · MBTI 매칭 카드 (`/fun`에서 흡수)
- **생산성** (1) — 뽀모도로 타이머
...
### `/fun` 게임
- **게임** (15) — 반응속도 · 2048 · 지뢰찾기 · 카드 짝맞추기 · 스네이크 · 사이먼 · 숫자야구 · 워터소트 퍼즐 · 타워 쌓기 · 블록 블라스트 · 매치3 퍼즐 · 벽돌깨기 · 두더지잡기 · 장애물 피하기 · 슬라이딩 퍼즐. 게임 자체는 프론트 로컬이지만, 로그인 사용자의 점수는 전부 백엔드 `game` 모듈(세션 토큰·레이트리밋·물리적 상한 검증)로 제출돼 리더보드에 반영된다. (재미 카테고리는 생활 도구로 흡수돼 이 구역은 게임 전용)
