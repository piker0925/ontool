...
| 🟪 | **`/fun` 재미·게임** | 로또 · 랜덤 도구 · 워드클라우드 · 룰렛 · 밸런스게임 · 미니게임 8종 | 19 |
...
3. **공통 카탈로그로 백엔드 33개·프론트 72개, 105개 도구를 통합한 확장 구조** — 백엔드 도구는 `ToolModule`을 구현하는 `@Component` 클래스 하나만 추가하면 자동 등록되고, 순수 계산으로 끝나는 나머지 72개는 백엔드를 거치지 않고 프론트에서 바로 처리된다. 성격이 다른 파일 처리·즉석 계산·게임·문서 뷰어가 같은 카탈로그·검색·통계 위에 공존하되, 행위 공유는 일부러 하지 않아(과추상화 방지) 데이터 계층만 통합했다. ([설계 →](docs/adr/0026-page-module-generalization.md))
...
### `/fun` 재미·게임 (전부 프론트 로컬)
- **재미** (11) — 로또 번호 생성 · 로또 시뮬레이터 · 랜덤 팀/사다리타기 · 랜덤 닉네임 · 색상 팔레트 생성기 · 워드클라우드 · 룰렛 돌림판 · 순서 정하기 · 핀볼 추첨기 · 밸런스 게임 · 아이스브레이킹 질문
...
**관리자 페이지**
`/admin/stats`(전체 통계) · `/admin/suggestions`(건의사항) · 댓글 삭제 · 유저 목록(refresh token 재사용 감지 발동 횟수 컬럼 포함, 탈취 확정 아닌 참고용 빈도 지표) · 유저 강제 로그아웃 · **계정 강제 삭제**(비가역 — 자체 확인 모달 필수, 회원 탈퇴와 동일하게 개인화 데이터 삭제·댓글 익명화) · 관리자 행위 감사로그(강제 로그아웃·댓글 삭제·계정 강제 삭제 등 "언제 무엇을" 기록). HTTP Basic Auth 보호.
