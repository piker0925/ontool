...
1. **외부 인프라 없이 MySQL만으로 만든 비동기 작업 큐** — Redis·RabbitMQ 없이 `Job` 테이블 + `SELECT ... FOR UPDATE SKIP LOCKED`로 다중 워커가 동시에 꺼내가도 중복 처리 없는 큐를 구현하고, **레인(Lane) 기반 동시성 제어**(영상 인코딩=동시 1, 짧은 작업=동시 2)와 소유자별 공정 스케줄링·용량 인지 admission으로 2코어 무료 VM에서도 무거운 작업이 큐를 막지 않게 했다. ([설계 →](docs/adr/0019-job-scheduler-resource-model.md)) — **실측**: 소유자 2명까지는 공정 스케줄링이 정확히 작동(뒤에 온 소유자가 앞선 40건 백로그와 무관하게 2번째 틱에 서비스됨)하고, 과부하 시 503/507로 즉시·깔끔하게 거절됨을 로컬 벤치마크로 확인했다 — 단 permit 수보다 소유자가 많을 때의 경계(→ [트러블슈팅](#트러블슈팅))도 같은 실험에서 발견했다. ([방법론·원본 데이터 →](docs/benchmarks/103-heavy-queue-bench/README.md))
...
- **공정 스케줄링 + 쿼터**: 한 사용자가 대량으로 밀어넣어도 다른 사용자가 굶지 않도록 소유자별 라운드로빈으로 꺼내고, 전역 용량(디스크 예산·큐 깊이)을 문 앞에서 검사해 초과 시 거절한다(ADR-0019, ADR-0021). **실측(로컬, 127 수정 전 baseline)**: HEAVY 레인(permit=2)에서 소유자 2명은 공정성이 정확히 성립(뒤에 온 소유자가 40건 백로그와 무관하게 2번째 틱에 서비스)하지만, 소유자 3명부터는 permit 수보다 owner 수가 많아 뒤쪽 owner가 앞선 두 owner의 백로그(약 60초 동안 40건)가 전부 빌 때까지 단 한 틱도 선택되지 못했다 — 자세한 경위는 [트러블슈팅](#트러블슈팅) 참고. 과부하 시에는 큐 깊이 상한을 넘는 요청이 정확히 12번째에서 503으로, 디스크 예산 초과는 507로 즉시·깔끔하게 거절됨을 확인했다(시스템이 느려지거나 큐가 무한정 쌓이지 않음). ([방법론·원본 데이터 →](docs/benchmarks/103-heavy-queue-bench/README.md))
...
이 발견은 **별도 이슈 127**(라운드로빈 소유자 확장 수정)로 분리했다 — 이 벤치마크의 시나리오 2 결과가
127의 **before 증거**이고, 127 머지 후 같은 하네스로 재측정한 after 수치를 이 문서에 추가할 예정이다.
