...
3. **공통 카탈로그로 백엔드 37개·프론트 90개, 127개 도구를 통합한 확장 구조**
   ...
   - **의도적으로 안 합친 것**: 순수 계산으로 끝나는 나머지 90개는 백엔드를 거치지 않고 프론트에서 바로 처리한다. 파일 처리·즉석 계산·게임·문서 뷰어처럼 성격이 다른 도구들을 같은 카탈로그·검색·통계 위에 공존시키되, 처리 로직(행위)까지 억지로 공유 인터페이스로 묶지 않고 데이터 계층(메타데이터·통계)만 통합했다(과추상화 방지). ([설계 →](docs/adr/0026-page-module-generalization.md))
