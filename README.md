...
3. **공통 카탈로그로 백엔드 37개·프론트 90개, 127개 도구를 통합한 확장 구조** — 백엔드 도구는 `ToolModule`을 구현하는 `@Component` 클래스 하나만 추가하면 자동 등록되고, 순수 계산으로 끝나는 나머지 90개는 백엔드를 거치지 않고 프론트에서 바로 처리된다. 성격이 다른 파일 처리·즉석 계산·게임·문서 뷰어가 같은 카탈로그·검색·통계 위에 공존하되, 행위 공유는 일부러 하지 않아(과추상화 방지) 데이터 계층만 통합했다. ([설계 →](docs/adr/0026-page-module-generalization.md))
...
> **테스트**: 백엔드 790+ (JUnit 5 · Testcontainers MySQL · Awaitility) · 프론트엔드 1,870+ (vitest). 주요 결정은 [ADR 36건](docs/adr/)으로 기록. DB 인덱스 감사(누락 인덱스를 실제 MySQL에서 `EXPLAIN`으로 검증)는 [별도 문서](docs/benchmarks/108-db-index-audit/README.md) 참고.
