...
`ToolModule`에 `default Lane getLane() { return Lane.HEAVY; }`를 추가. `Lane`은 `HEAVY`/`VIDEO`. 비디오 모듈(이슈 037)만 `VIDEO`로 오버라이드한다.
...
## 지금 구현한 범위 vs 대용량·비디오(036·037)로 미룬 것
...
**의도적으로 미룸 (아직 코드 없음 — 오해 방지용 명시. 인프라는 036, 비디오 기능은 037)**:

- **모듈→진행 보고 콜백**: 현재 `ToolModule.process(ToolInput)`은 끝에 결과 하나만 반환한다. 처리 중간에 `progress`를 갱신할 통로가 없어 **진행률 바·ETA는 지금은 비활성**(단건은 0→100 점프). 비디오 모듈이 FFmpeg 진행을 파싱해 `job.setProgress()`를 주기적으로 부르려면 `process()`에 진행 콜백을 추가해야 하며, 대용량 처리 인프라(이슈 036) 스코프로 둔다.
- **작업 타임아웃 + 외부 프로세스 kill**: 망가진 영상이 FFmpeg를 무한 점유하는 걸 끊는 장치. 비디오 없이는 불필요.
- **스트리밍 I/O 강제**: 대용량 파일을 `byte[]`로 올리지 않고 디스크 경로로만 다루는 원칙. 대용량 처리 인프라(036)에서 강제.
- **디스크 예산·대용량 경고**: 총 업로드+결과 용량 상한, 큐 깊이 기반 앞단 거부. 실제 블록 스토리지 용량(`df -h`) 확인 후 036에서 수치 확정. (업로드 한도 자체는 032.)
