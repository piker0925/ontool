...
| 🟪 | **`/fun` 재미·게임** | 로또 · 랜덤 도구 · 워드클라우드 · 룰렛 · 밸런스게임 · 미니게임 8종 | 21 |
...
3. **공통 카탈로그로 백엔드 37개·프론트 86개, 123개 도구를 통합한 확장 구조** — 백엔드 도구는 `ToolModule`을 구현하는 `@Component` 클래스 하나만 추가하면 자동 등록되고, 순수 계산으로 끝나는 나머지 86개는 백엔드를 거치지 않고 프론트에서 바로 처리된다. 성격이 다른 파일 처리·즉석 계산·게임·문서 뷰어가 같은 카탈로그·검색·통계 위에 공존하되, 행위 공유는 일부러 하지 않아(과추상화 방지) 데이터 계층만 통합했다. ([설계 →](docs/adr/0026-page-module-generalization.md))
...
> **테스트**: 백엔드 710+ (JUnit 5 · Testcontainers MySQL · Awaitility) · 프론트엔드 1,386+ (vitest). 주요 결정은 [ADR 34건](docs/adr/)으로 기록.
...
- **게임** (8) — 반응속도 · 2048 · 지뢰찾기 · 카드 짝맞추기 · 스네이크 · 사이먼 · 숫자야구 · 틱택토. 게임 자체는 프론트 로컬이지만, 로그인 사용자의 점수는 백엔드 `game` 모듈(세션 토큰·레이트리밋·물리적 상한 검증)로 제출돼 리더보드에 반영된다.
